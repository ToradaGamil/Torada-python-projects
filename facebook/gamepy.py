...
class Brick:
    ...
    def update(self):
        ...
        glBegin(GL_QUADS)
        glVertex(self.x - self.width, self.y - self.height, self.z)  # Left - Bottom
        glVertex(self.x + self.width, self.y - self.height, self.z)
        glVertex(self.x + self.width, self.y + self.height, self.z)
        glVertex(self.x - self.width, self.y + self.height, self.z)
        glEnd()

    def hit(self):
        if (g.bl.y - g.bl.radius <= self.y + self.height and g.bl.y - g.bl.radius >= self.y - self.height) or (
                g.bl.y + g.bl.radius >= self.y - self.height and g.bl.y + g.bl.radius <= self.y + self.height):
...
class ball:
    ...
    def playercol(self):
        if (self.y - self.radius <= g.p.top) and (self.x - self.radius <= g.p.right) and (
                self.x + self.radius >= g.p.left):
...
class borders:
    def __init__(self):
        ...
        self.line_3_dash = [(viewport[0] - wid), 45, (viewport[0] - wid), 300, viewport[0], 300, viewport[0], 45, 1, 1,
                            1]
        self.line_4 = [0, 300, 0, 400, wid, 400, wid, 300, 1, 1, 0]
        self.line_4_dash = [(viewport[0] - wid), 300, (viewport[0] - wid), 400, viewport[0], 400, viewport[0], 300, 1,
                            1, 0]
        self.line_5 = [0, 400, 0, 500, wid, 500, wid, 400, 0, 1, 0]
        self.line_5_dash = [(viewport[0] - wid), 400, (viewport[0] - wid), 500, viewport[0], 500, viewport[0], 400, 0,
                            1, 0]
        self.line_6 = [0, 500, 0, 600, wid, 600, wid, 500, 1, (128 / 255), 0]
        self.line_6_dash = [(viewport[0] - wid), 500, (viewport[0] - wid), 600, viewport[0], 600, viewport[0], 500, 1,
                            (128 / 255), 0]
        self.line_7 = [0, 600, 0, viewport[1], wid, viewport[1], wid, 600, 0, 0, 1]
        self.line_7_dash = [(viewport[0] - wid), 600, (viewport[0] - wid), viewport[1], viewport[0], viewport[1],
                            viewport[0], 600, 0, 0, 1]
        self.top_line = [wid, viewport[1], (viewport[0] - wid), viewport[1], (viewport[0] - wid), (viewport[1] - wid),
                         wid, (viewport[1] - wid), 0, 0, 1]
...
def init():
    ...
    glLoadIdentity()
    glOrtho(0, (viewport[0]), 0, (viewport[1]), -2, 2)  # l,r,b,t,n,f
...
def Display():
    ...
    '''
        ...
        #glClear(GL_COLOR_BUFFER_BIT)
        #glColor(1,1,1)
        Text("GAME OVER", (viewport[0]/2) - 155, viewport[1]/2, 3,1, 0, 0)
        Text("Press E to Exit or R to restart", (viewport[0]/2) - 140, (viewport[1]/2)-40)
        del g
        g = Game()
    '''
...
def main():
    ...
    glutInitWindowSize(viewport[0], viewport[1])  # mouse coordinates inbetween [WINDOW_WIDTH=800,WINDOW_HEIGHT=500]
